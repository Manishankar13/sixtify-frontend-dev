  ...
  const columns: AgColumnsWithActions<IncomeTaxRegimePayload> = [
    ...
    },
    {
      ...
      cellRenderer: ({ value }: CustomCellRendererProps) => {
        ...
        if (value) {
          const startYear = DateTime.fromISO(value.start_date).toFormat(
            "LLLL yyyy"
          );

          const endYear = DateTime.fromISO(value.end_date).toFormat(
            "LLLL yyyy"
          );
    ...
    },
    {
      ...
      cellRenderer: ({
        data,
      }: CustomCellRendererProps<IncomeTaxRegimePayload>) => {
        if (loading) {
          return <LoadingCell />;
        }
