...
export const useTaxesDeductionsTabOption = () => {
  const router = useRouter();

  const categoryItems: TabsItems[] = [
    {
      value: "income-tax-regime",
      title: "Income Tax Regime",
      onClick: () =>
        router.push(
          `/payroll/settings/taxes-deductions?tab=income-tax-regime`
        ),
    },
    {
      value: "pt-group",
      ...
      onClick: () =>
        router.push(
          `/payroll/settings/taxes-deductions?tab=pt-group`
        ),
    },
    {
